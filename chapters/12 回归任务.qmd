...
# 目标变量分布
axes[0,0].hist(diabetes_data['target'], bins=30, color='steelblue', alpha=0.7)
axes[0,0].set_title('疾病进展分布')
axes[0,0].set_xlabel('疾病进展指标')
axes[0,0].set_ylabel('频数')

# 疾病进展与关键变量的关系
axes[0,1].scatter(diabetes_data['bmi'], diabetes_data['target'], alpha=0.6, color='darkred')
z = np.polyfit(diabetes_data['bmi'], diabetes_data['target'], 1)
p = np.poly1d(z)
...
axes[1,0].scatter(diabetes_data['s5'], diabetes_data['target'], alpha=0.6, color='darkgreen')
z = np.polyfit(diabetes_data['s5'], diabetes_data['target'], 1)
p = np.poly1d(z)
...
axes[1,1].scatter(diabetes_data['age'], diabetes_data['target'], alpha=0.6, color='purple')
z = np.polyfit(diabetes_data['age'], diabetes_data['target'], 1)
p = np.poly1d(z)
...
1.  **医疗诊断**：使用训练好的模型对患者疾病进展进行预测
2.  **治疗方案制定**：识别影响疾病进展的关键因素，指导治疗策略
3.  **患者管理**：理解生理指标对疾病的影响，支持患者管理
4.  **风险评估**：监测疾病进展风险，预警健康问题
